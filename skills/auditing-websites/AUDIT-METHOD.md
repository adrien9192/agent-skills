...
**The doctrinal anchor, before any GEO finding.** Google's own AI-features page (updated 2025-12-10) states the eligibility rule in one sentence: "To be eligible to be shown as a supporting link in AI Overviews or AI Mode, a page must be indexed and eligible to be shown in Google Search with a snippet, fulfilling the Search technical requirements. **There are no additional technical requirements.**" It adds that "specific optimization isn't required" and lists ordinary fundamentals — crawlable, internally linked, important content in textual form, structured data matching the visible text. Everything in this section is therefore an *agent-readability* layer sitting on top of a correct site, never a substitute for one and never a documented ranking lever. An audit that scores a site down for missing GEO plumbing while its titles, internal links and indexability are broken has inverted the priority.

- `llms.txt` (summary) + `llms-full.txt` (full prose) in **standard markdown** (no proprietary format), generated from the same data as the pages. **Audit it as agent-readability, never as a ranking signal, and never score its absence.** Google documents that the file has no effect, positive or negative, on rankings or AI Overviews. Where it does work is the agent layer — coding assistants and MCP clients reading a documentation — which is why OpenAI, Anthropic and Perplexity each serve one for their own developer docs (HTTP 200, checked 2026-08-09) while `developers.google.com/llms.txt` returns 404. OpenAI's own crawler page opens on "For the complete documentation index, see llms.txt. Markdown versions of documentation pages are available by appending `.md` to the page URL" — that pairing, not the bare file, is the mechanism: the `<link rel="alternate" type="text/markdown">` and the `.md` twin are what an agent follows.
  [WARN] The widely-quoted log studies ("515M bot requests, llms.txt essentially never fetched") filtered on **GPTBot and ClaudeBot as citation proxies**, and both are documented **training** crawlers. They measured the wrong lane. Cite them as directional at most; the load-bearing evidence against llms.txt-as-ranking is Google's own documentation plus the `cats.txt` falsifier (§8), neither of which depends on a user-agent taxonomy.
...
- robots.txt: **audit AI bots by role, never as one flat list** — the blanket "block the AI crawlers" posture is now a visibility bug. Each vendor separates three jobs and blocking one does not block the others:
  - **Search / answer indexing** — `OAI-SearchBot`, `Claude-SearchBot`, `PerplexityBot`. These decide whether the site can be shown in ChatGPT, Claude and Perplexity answers. OpenAI states it plainly: "Sites that are opted out of OAI-SearchBot will not be shown in ChatGPT search answers, though can still appear as navigational links." The loss is the *answer*, not every trace of the site. A `Disallow` here is a P1 finding on any site that says it wants AI visibility.
  - **Google is not in that list, and this is the most common mistake on the axis.** Google's AI-features page says AI Overviews and AI Mode ride Search itself: "robots.txt directives for **Googlebot** is the control for site owners to manage access to how their sites are crawled for Search", and the way to limit what is shown is `nosnippet`, `data-nosnippet`, `max-snippet` or `noindex`. There is no AI-Overviews-specific user agent to allow. `Google-Extended` is a **separate** opt-out covering "AI training and grounding in some of Google's other systems" — blocking it does not remove a site from AI Overviews, and allowing it does not buy a place in them. Report it as its own editorial choice and never attach an AI-Overviews promise to it.
  - **Training** — `GPTBot`, `ClaudeBot`, `anthropic-ai`, `CCBot` (Common Crawl, training-only). Blocking these is a legitimate editorial choice with no documented search cost; OpenAI explicitly supports the split ("allow OAI-SearchBot in order to appear in search results while disallowing GPTBot"). Report the choice, never grade it.
  - **User-initiated fetch** — `ChatGPT-User`, `Claude-User`, `Perplexity-User`. Fired when a person asks about the page. OpenAI notes it "is not used to determine whether content may appear in Search"; Anthropic states all three of its agents honour robots.txt.
  Plus Content-Signal. Check the dated vendor pages at audit time: this taxonomy changed in February 2026 when Anthropic split one crawler into three, and it will change again.
...
### 8 bis. Dating the algorithm landscape

Never state the ranking-update landscape from model memory: it is the single fact in this document that is guaranteed stale, and an agent asked for "the recent updates" will confidently reproduce the calendar it was trained on. One scout on this repo returned "no Google announcement after August 2024" while the dashboard listed nine subsequent updates.

Take it from the **Search Status Dashboard** ranking history, which is the authoritative, dated list (it also exposes `incidents.json` for scripting). Read on 2026-08-09, it gave: June 2026 spam update (24 Jun), May 2026 core (21 May), March 2026 core (27 Mar), March 2026 spam (24 Mar), February 2026 Discover (5 Feb), December 2025 core (11 Dec), August 2025 spam (26 Aug), June 2025 core (30 Jun), March 2025 core (13 Mar). Roughly one core update per quarter plus a spam update between them, each running one to four weeks.

Two consequences for an audit. A traffic drop is dated against that list before any other hypothesis, and "the site has not recovered" is meaningless until at least one full core update has run since the fix. And the file's own collection date is written next to the table, because this section rots by construction.
