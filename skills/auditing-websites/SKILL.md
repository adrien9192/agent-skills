...
5. **Quality review**: design anti-slop, seven-category socratic review, CRO copy, images and performance → `QUALITY-REVIEW.md`. On the accessibility category, carry the WCAG criterion ID on every finding and run the manual pass an automated scan cannot (keyboard-only traversal, screen-reader announcement, measured contrast ratio, 200% zoom): protocol in `accessibility-review`. The criterion ID is also what gives an a11y finding a falsifiable `verify_cmd`, so it enters reconciliation instead of bypassing it. Automated QA: `assets/audit-local.mjs` (generic — set the `AUDIT_*` variables). When `[data-scroll-experience]` exists or the site plan requires a scroll timeline, also run `assets/audit-scroll.mjs` on desktop, mobile and reduced motion, then read each contact sheet.
   *Done when*: the seven socratic categories are through, `audit-local.mjs` has run on all three breakpoints, and every triggered scroll pass has its report and reviewed contact sheet attached.
...
| On `llms.txt`, report presence; never score its absence. | "The bots fetched it, so it works" — an invented `cats.txt` describing someone's office cats drew PerplexityBot, GPTBot, ClaudeBot and Googlebot, got indexed, and ChatGPT recommended it in the same words used to pitch `llms.txt`. A probe that a fake standard passes cannot fail differently. |
| Google's AI-features eligibility rule is quoted, not paraphrased. | "AI Overviews need special markup" — Google's own page says a page must be indexed and snippet-eligible, and that "there are no additional technical requirements". Selling a GEO prerequisite Google explicitly denies is the fastest way to lose an audit's credibility. |
| Audit AI crawlers by role: search, training, user-fetch. | "The site blocks the AI bots, that's a privacy choice" — blocking `GPTBot` blocks training only. `OAI-SearchBot`, `Claude-SearchBot` and `PerplexityBot` are what decide whether the site can appear in ChatGPT, Claude and Perplexity answers, and OpenAI documents that opting out of `OAI-SearchBot` keeps the site out of ChatGPT search answers, though it can still appear as a navigational link. One flat list conflates a legitimate editorial choice with a P1 visibility bug. |
