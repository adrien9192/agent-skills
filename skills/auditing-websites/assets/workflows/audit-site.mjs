...
const AXES = [
  {
    ...
    prompt: `Tu audites ${domain} sur l'axe SEO (technique + sémantique). Lis d'abord ${SKILL}/AUDIT-METHOD.md sections 1, 3, 4, 6 et ${SKILL}/ORCHESTRATION.md sections 2 et 4 (protocole curl). ${repoNote} ${modeNote} Collecte : indexabilité par page, canonical/hreflang, meta (unicité, 50-170c), doublons sur même intention (candidats consolidation), maillage interne (plancher de liens), pages orphelines de la nav, sitemap (lastmod statiques ?). ${CONTRAT}`,
  },
  {
    key: 'geo', model: 'haiku',
    prompt: `Tu audites ${domain} sur l'axe GEO (agent-readiness). Lis d'abord ${SKILL}/AUDIT-METHOD.md section 5 et ${SKILL}/ORCHESTRATION.md section 4. Batterie curl : /llms.txt et /llms-full.txt ; négociation markdown (curl -H 'Accept: text/markdown' + header Vary) ; /robots.txt — résoudre les GROUPES avec la commande : curl -s <domaine>/robots.txt | awk -f ${SKILL}/assets/robots-roles.awk — (la RFC 9309 laisse plusieurs user-agent consécutifs partager les règles qui suivent : un grep de contexte perd la directive d'un bloc groupé), puis classer PAR RÔLE (AUDIT-METHOD §5), jamais en liste plate, la directive Allow/Disallow RÉELLE faisant foi et jamais la seule présence du nom : **recherche/citation** (OAI-SearchBot, Claude-SearchBot, PerplexityBot) — un Disallow ici est un finding P1 sur un site qui dit vouloir de la visibilité IA, OpenAI documentant qu'un opt-out d'OAI-SearchBot retire le site des réponses de ChatGPT search, même s'il peut encore apparaître en lien de navigation — la perte porte sur la réponse, pas sur toute trace du site ; **entraînement** (GPTBot, ClaudeBot, anthropic-ai, CCBot) — choix éditorial sans coût de recherche documenté, à rapporter, jamais à noter ; **fetch déclenché par un humain** (ChatGPT-User, Claude-User, Perplexity-User) ; **Google à part** — AI Overviews et AI Mode passent par Googlebot, Google-Extended est un opt-out distinct dont le blocage ne retire pas des AI Overviews et dont l'autorisation n'y achète pas de place, ne jamais lui attacher une promesse AI Overviews. Plus Content-Signal (header de la réponse robots.txt ET/OU ligne dans le corps) ; RSS ; JSON-LD bruts par famille de page ; date de mise à jour VISIBLE dans le body. Chaque item : présent/absent/malformé + extrait de preuve. RÈGLE DURE sur llms.txt : présence RAPPORTÉE, jamais notée — son absence n'est pas un défaut et sa présence n'est pas un gain de classement (AUDIT-METHOD §5) ; ne jamais citer les études de logs « 515M requêtes », elles filtrent sur GPTBot et ClaudeBot, des crawlers d'ENTRAÎNEMENT, et mesurent donc la mauvaise voie.${compareTo ? ` MODE COMPARE : rejoue exactement la même batterie sur ${compareTo} et produis le diff — ce que la version actuelle a PERDU par rapport à l'ancienne est un finding par élément perdu (cas réel : un site a perdu toute sa couche GEO en refondant).` : ''} ${CONTRAT}`,
  },
  {
    key: 'design', model: 'sonnet',
    prompt: `Tu audites ${domain} sur l'axe design/UX anti-slop. Lis d'abord ${SKILL}/QUALITY-REVIEW.md section 1 (grille quantifiée + faux positifs) et ${SKILL}/ORCHESTRATION.md section 2. ÉTAPE BLOQUANTE avant tout jugement : déterminer le design read (preserve : identité documentée à protéger / redesign) ${repoPath ? `en cherchant le doc de marque dans ${repoPath}` : 'en observant la cohérence délibérée du site'}. Captures Playwright 390/768/1440 par famille de page si possible (forcer content-visibility:visible avant), sinon analyse du HTML/CSS par curl. Vérifie la hiérarchie RÉELLE des balises (h1-h3 dans le code, pas le rendu). Sépare VALIDE (intentionnel) de FLAG (slop). ${CONTRAT}`,
  },
  {
    key: 'cro', model: 'sonnet',
    prompt: `Tu audites ${domain} sur l'axe CRO/copywriting. Lis d'abord ${SKILL}/QUALITY-REVIEW.md section 3 et ${SKILL}/ORCHESTRATION.md section 2. Pour chaque page clé (curl) : pose D'ABORD l'objectif de conversion primaire unique (introuvable = premier finding) ; chasse les 5 frictions ; parcours complet vers la conversion (CTA visibles, formulaire fonctionnel, téléphone cliquable mobile, réassurance à côté du formulaire). Règle dure : chaque finding inclut la réécriture proposée dans le champ preuve — jamais "améliorer le copy" ; donnée manquante (prix, délai, témoignage) = finding marqué [GATE CLIENT] dans le claim. ${CONTRAT}`,
  },
  {
    key: 'code', model: 'sonnet',
    prompt: `Tu audites ${domain} sur l'axe code/perf/a11y/confiance. Lis d'abord ${SKILL}/QUALITY-REVIEW.md sections 2, 4, 5 et ${SKILL}/ORCHESTRATION.md section 4. ${repoNote} Collecte : images (WebP, dimensions, priority/lazy selon LCP, hotlinks), headers sécurité/CSP, formulaires (anti-spam, rate-limit, payload invalide sur les routes API — pas de crash), TRACKING RÉEL vs politique de confidentialité (mismatch = P0 isolé, jamais noyé dans un lot cosmétique), erreurs console. Pièges : contraste axe-core faussé par content-visibility:auto ; scan à 0 éléments = sélecteur obsolète, pas un succès. ${CONTRAT}`,
  },
]
...
return {
  ...
  suite_au_thread_principal: 'Scoring par axe (ORCHESTRATION.md §5), revue qualité (QUALITY-REVIEW.md), plan 4 paliers + livrable (ORCHESTRATION.md §6-7). Aucun INVERIFIABLE ne doit être promu.',
}
