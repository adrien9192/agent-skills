...
- **Declared files that were never written.** An agent returns a structured result listing the artefacts it produced, and none of them exists on disk. Measured three times in one session, on three different agents, including one that had done the research correctly and lost it at the yield. A claimed artefact is a claim: `ls` it before consuming it, and ask for the content **in the reply body** when the agent is read-only.
- **A mid-flight correction acknowledged and then ignored.** One agent replied "understood, dropping the quota" and shipped a section titled *additional pages to reach 42*. Re-state constraints as acceptance criteria the agent must echo back with measured values, not as prose it can agree with.
- **An invented credential.** An agent wrote a certification count into a plan, i.e. fabricated exactly the qualifying criterion of a partner programme. Any status, certification, partnership or headcount is client data: it comes from the client in writing or it does not ship.
- **Reports that normalise accents in their own summary.** An agent's JSON summary showed unaccented titles while the file it wrote was correctly accented. Audit the artefact, never the report about the artefact — otherwise you will "fix" a defect that does not exist.
...
45. **Third-party logos: the form decides, not the intent.** Using a technology's logo to identify the subject of a page is nominative use and is standard. Turning it into a faded background is an **alteration** of colour and opacity, which several trademark policies forbid outright even where the logo itself may be shown. Read each project's own trademark page before downloading: some require prior written permission (measured on one CMS: "You must have permission [...] to use the name and the logo"). Self-host, keep the file unaltered, carry its **intrinsic dimensions** — a wordmark forced into a square is squashed — and record the source URL next to the asset.
46. **`dangerouslyAllowSVG` is not a blocker for local logos.** The flag governs *remote* SVG passed through the image optimiser. Files served from the public directory are delivered untouched with no flag, which is how existing SVG logos already render. Do not refuse an asset on this ground without checking what the constraint actually covers.
