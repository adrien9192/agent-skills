...
description: "Use when producing the public marketing layer of a site after architecture and source ownership are set, or for a standalone greenfield marketing site with no upstream controller. Covers showcase sites, landing pages, local SEO, lead generation, semantic silos, GEO readiness, content truth, accessibility, and pre-launch quality. Not for full product delivery, live-site extraction alone, SaaS UI, or audit-only work."
---
...
5. **Content**: apply `EEAT-WRITING.md`. At 3+ pages, use parallel agents when available (BLUEPRINT §13: batches of 2-4 pages, strong reasoning for editorial work and the fast tier for mechanical volume, prompt = exact format + §7 rules + a cleaned reference file, then inspect files to confirm — an agent can report DONE without writing). Then page by page: **REQUIRED SUB-SKILL** `humanizer` on the copy, THEN the gate — figures, testimonials, prices and seniority go through `site-fact-checker`, traced to INTAKE-CLIENT.md or a named source.
