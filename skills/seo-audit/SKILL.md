...
When the deliverable introduces or evaluates a factual claim, number, quote,
testimonial, competitive comparison, performance prediction, or customer fact,
load `marketing-evidence` before delivery. Maintain its claim ledger and mark
each claim `verified`, `contradicted`, `unverified`, or `expired`. Only
`verified` claims may be presented as facts.

A purely stylistic rewrite may reuse user-provided claims without new research,
but keep them `unverified` with a user-provided limitation until a source
verifies them.
